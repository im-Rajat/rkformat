...
The suite covers round-tripping, determinism, reference resolution (including code-fence
masking), integrity checks, and the security limits above. Image fixtures are synthesised
with the standard library, so there are no binary files in the repo.
