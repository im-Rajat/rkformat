...
// Shared with the web editor: assigns RKF.toolbar onto the global object.
require("./media/toolbar.js");
const { icon, formatBarHtml } = globalThis.RKF.toolbar;
...
/**
 * The webview's body markup.
 *
 * Kept as a standalone function, not inlined into buildHtml, so a test can render the exact
 * markup that ships. `tests/make_live_harness.py` calls it through Node.
 */
function editorBodyHtml() {
  return `<body class="rkf-editor">
  <header id="toolbar">
    ...
    <span class="sep"></span>
    <button class="tool" id="toggle-assets" title="Show embedded images">${icon("gallery")}</button>
    <button class="tool" id="cleanup" title="Delete images the body no longer references">${icon("sweep")}</button>
    <button class="tool" id="toggle-width" title="Toggle page width">${icon("page")}</button>
    <div class="spacer"></div>
    <div id="status" aria-live="polite"></div>
  </header>
  <div id="format-bar" role="toolbar" aria-label="Formatting" hidden>${formatBarHtml()}</div>
  <main id="panes">
    ...
    switch (message.type) {
      case "ready":
        panel.webview.postMessage({
          ...
          width: settings().get("editorWidth"),
      ...
      case "history":
        // VS Code owns the undo stack (see applyEdit), so the toolbar buttons have to ask it
        // rather than trying to keep a second history in the webview.
        await vscode.commands.executeCommand(message.direction === "redo" ? "redo" : "undo");
        return;
    }
  }
...
module.exports = { activate, deactivate, editorBodyHtml };
